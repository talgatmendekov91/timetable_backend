...
const startTelegramNotifications = () => {
  ...
  cron.schedule('*/10 * * * *', () => {
    console.log('ðŸ”” Checking for upcoming lessons...');
    notifier.checkUpcomingLessons();
  });

  console.log('âœ… Telegram notification cron job started (every 10 minutes)');
};

const stopTelegramNotifications = () => {
  if (notifier) {
    notifier.stop();
    console.log('âŒ Telegram notifications stopped');
  }
};
